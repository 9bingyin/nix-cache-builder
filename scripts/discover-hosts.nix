...
else
  {
    ...
    summary = concatStringsSep "\n" (
      [
        ...
        "Revision: `${flakeRev}`"
        ""
        "Discovery only reads configuration names. Ubuntu evaluates each target system without IFD first."
        "A failed system group falls back to its native runner, where IFD is allowed."
